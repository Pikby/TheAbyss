...
int Polygonise(const GRIDCELL& grid, const glm::vec3* p,const double isolevel,TRIANGLE *triangles)
{
    ...
    for(int x = 0;x<CHUNKSIZE;x++)
    {
      for(int z = 0;z<CHUNKSIZE;z++)
      {
        for(int y = 0;y<CHUNKSIZE;y++)
        {
          ...
          {
            glm::vec3(0,0,punit),
            glm::vec3(punit,0,punit),
            glm::vec3(punit,0,0),
            glm::vec3(0,0,0),
            glm::vec3(0,punit,punit),
            glm::vec3(punit,punit,punit),
            glm::vec3(punit,punit,0),
            glm::vec3(0,punit,0),
          };
          ...
          static const glm::vec3 subCubeLookup[8]=
          {
          ...
          for(int subCubes=0;subCubes<8;subCubes++)
          {
            ...
            if(ntris == 2)
            {
              ...
              for(int i=0;i<3;i++)
              {
                auto itr = std::find(list.begin(),list.end(),tris[0].p[i]);
                if(itr == list.end())
                {
                  list.push_back(tris[0].p[i]);
                }
              }
              for(int i=0;i<3;i++)
              {
                auto itr = std::find(list.begin(),list.end(),tris[1].p[i]);
                if(itr == list.end()) list.push_back(tris[1].p[i]);
              }
              int startId;
              if(list.size() == 4)
              {

                vertex.ids[0] = list[0].id;
                vertex.ids[1] = list[1].id;
                vertex.ids[2] = list[2].id;
                for(int i=0;i<4;i++)
                {
                  vertex.pos = list[i].pos + glm::vec3(chunkLocalPos) + subCubeLookup[subCubes]/float(lod);
                ...
                for(int i=0;i<3;i++)
                {
                  for(int j=0;j<4;j++)
                  {
                    if(list[j] == tris[0].p[i]) oIndices.push_back(startId + j);
                  }
                }
                for(int i=0;i<3;i++)
                {
                  for(int j=0;j<4;j++)
                  {
                    if(list[j] == tris[1].p[i]) oIndices.push_back(startId + j);
                  }
                }
            ...
            */
            ...
            {
              glm::vec3 normal = glm::cross(tris[i].p[0].pos- tris[i].p[1].pos,tris[i].p[0].pos - tris[i].p[2].pos);
              vertex.ids[0] = tris[i].p[0].id;
              vertex.ids[1] = tris[i].p[1].id;
              vertex.ids[2] = tris[i].p[2].id;
              for(int j=0;j<3;j++)
              {
                vertex.pos = tris[i].p[j].pos + glm::vec3(chunkLocalPos) + subCubeLookup[subCubes]/float(lod);
                int id = addVertex(vertex);
